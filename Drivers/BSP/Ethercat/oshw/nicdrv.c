 ...
 *非阻塞接收帧功能。 使用RX缓冲区和索引将读取帧与发送帧组合在一起。 为了补偿接收到的乱序帧，所有帧都存储在它们各自的索引缓冲区中。
 *如果先前在缓冲区中放置了一个帧，则该函数会从该缓冲区索引中检索它而不调用ec_recvpkt。 如果请求的索引不在缓冲区中，则调用ec_recvpkt来获取它。
  现在有三个选项，1没有帧读取，所以退出。 2帧读取但不是请求索引，存储在缓冲区并退出。 3帧读取匹配索引，存储在缓冲区中，在缓冲区状态下设置完成标志并退出。
 ...
 *冗余接收帧功能。 如果冗余模式未激活，则它会跳过辅助堆栈和冗余功能。 在冗余模式下，它等待（主要和次要）帧进入。结果在决策树中，根据数据包的路由及其可能的丢失到达，决定如何重新路由原始数据包以获取数据 在另一个尝试。
 * @param[in] port        = port context struct
 * @param[in] idx = requested index of frame
